...
class Pic extends Component {
    ...
    render() {
        return <div>
            <img style="margin:20px" src={this.state.src} alt="logo" width={this.state.size} height={this.state.size}></img>
            <div>
                <button onclick={() => {
                    let tmp = { sizeinc: this.props.id };
                    worker.postMessage(JSON.stringify(tmp));
                }}
                    style="margin-left:20px">Zoom in</button>
                <button onclick={() => {
                    let tmp = { sizedec: this.props.id };
                    worker.postMessage(JSON.stringify(tmp));
                }}
                    style="margin-left:20px">Zoom out</button>
            </div>
        </div>
    }
}
